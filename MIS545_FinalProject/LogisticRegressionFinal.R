...
setwd("/Users/namrathaacharya/Downloads/Sem 1/MIS 545 - Data mining/GroupProject")
...
# season, year, millennium week, streaming week - input
# success as output
  
#Remove the data plan and data usage variables from the tibble
# boxOfficePrePandemic <- select(boxOfficePrePandemic, -Gross)
...
sampleMobile<- sample(nrow(boxOfficePrePandemic),
                      round(nrow(boxOfficePrePandemic) * 0.75),
                      replace = FALSE)

# Put the records (75%) sample intp traning data set 
boxOfficePrePandemicTraining <- boxOfficePrePandemic[sampleMobile, ]

# Put the records (25%) sample into testing data set 
boxOfficePrePandemicTesting <- boxOfficePrePandemic[-sampleMobile, ]
...
# Accuracy
sum(diag(boxOfficePrePandemicConfusionMatrix)) / nrow(boxOfficePrePandemicTesting)
